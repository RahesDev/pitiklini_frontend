...
const assetRows = [
  {
    ...
    name: "Cardano",
    total: "5,759.24",
    totalUsdt: "=1950.08 USDT",
    available: "5,759.24",
    availableUsdt: "=1950.08 USDT",
    frozen: "0.00",
    frozenUsdt: "=0.00 USDT",
    valuation: "1950.08",
  },
  {
    symbol: "ETH",
    ...
    totalUsdt: "",
    available: "0.20",
    availableUsdt: "=466.39 USDT",
    frozen: "0.27",
    frozenUsdt: "=629.61 USDT",
    valuation: "1096.92",
  },
  {
    symbol: "BTC",
...
const CoinIcon = ({ symbol }) => {
    ...
    return (
      <span className={`${wrapper} bg-[#2a2d35]`}>
        <svg viewBox="0 0 32 32" className="h-6 w-6">
          ...
          <path d="M16 4 9.6 16 16 12.7 22.4 16 16 4Z" fill="#8C8C8C" />
          <path d="M16 12.7 9.6 16 16 19.8 22.4 16 16 12.7Z" fill="#62688F" />
          <path d="M16 27.5 9.6 17.2 16 21 22.4 17.2 16 27.5Z" fill="#454A75" />
        </svg>
      </span>
    );
  }
  if (symbol === "ADA") {
    return (
      <span className={`${wrapper} bg-[#142845]`}>
        <svg viewBox="0 0 32 32" className="h-6 w-6">
          <circle cx="16" cy="16" r="16" fill="#0A2B66" />
          {[
            [16, 8],
            [11, 10],
            [21, 10],
    ...
    return (
      <span className={`${wrapper} bg-[#312b1a]`}>
        <svg viewBox="0 0 32 32" className="h-6 w-6">
          ...
          <path
            d="M10 9.5h7.2c4 0 6.8 2.5 6.8 6.5s-2.8 6.5-6.8 6.5H10v-3.1h2V12.6h-2V9.5Zm5.2 10h1.7c2.1 0 3.5-1.3 3.5-3.5s-1.4-3.5-3.5-3.5h-1.7v7Z"
            fill="#fff"
          />
        </svg>
      </span>
    ...
    return (
      <span className={`${wrapper} bg-[#311823]`}>
        <svg viewBox="0 0 32 32" className="h-6 w-6">
          ...
          <path
            d="m8 10 16 3.3-7.7 10.8L8 10Zm3 2.2 5.3 8.6 5.4-6.8-10.7-1.8Zm2.4 1.3 7.1 1.2-3.4 4.3-3.7-5.5Z"
            fill="#fff"
          />
        </svg>
      </span>
  ...
  return (
    <span className={`${wrapper} bg-[#1b3950]`}>
      <svg viewBox="0 0 32 32" className="h-6 w-6">
        <circle cx="16" cy="16" r="16" fill="#2EA7F2" />
        <path
          d="M22.8 16c0 4.3-3.1 7.3-7.4 7.3-4.2 0-7.2-3-7.2-7.2 0-4.2 3-7.2 7.2-7.2 2 0 3.8.7 5.1 2l-1.8 1.8a4.8 4.8 0 0 0-3.4-1.3 4.8 4.8 0 0 0-4.8 4.8 4.8 4.8 0 0 0 4.9 4.9 4.3 4.3 0 0 0 4.3-2.7h-4.3V16h7.4Z"
          fill="#fff"
        />
      </svg>
    </span>
...
const AssetsTableSection = () => {
  ...
  const filtered = useMemo(() => {
    const searched = assetRows.filter((row) =>
      `${row.symbol} ${row.name}`.toLowerCase().includes(query.toLowerCase()),
    );
    const zeroFiltered = hideZero
      ? searched.filter((row) => row.total !== "0.00")
      : searched;
    if (!simplified) return zeroFiltered;
    return zeroFiltered.map((row) => ({
      ...row,
      availableUsdt: "",
      frozenUsdt: "",
      totalUsdt: "",
  ...
  return (
    <section className="mt-5 rounded-xl border border-[#252a36] bg-black p-5 shadow-[0_12px_30px_rgba(0,0,0,0.35)] md:p-6">
        ...
        <div className="flex flex-wrap items-center gap-3 md:gap-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search"
              className="h-10 w-52 rounded-xl border border-[#2a3038] bg-[#1a1f2a] pl-9 pr-3 text-sm text-[#BD7F10] placeholder:text-[#BD7F10] shadow-inner outline-none transition focus:border-[#BD7F10] focus:ring-2 focus:ring-[#BD7F10]/30 md:w-56"
            />
          </div>
          <label className="flex items-center gap-2 text-xs text-[#BD7F10] md:text-sm">
            <input
              ...
              className="h-4 w-4 rounded border border-[#2a3038] bg-[#1a1f2a] align-middle accent-[#BD7F10]"
            />
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[920px] w-full border-separate border-spacing-y-2">
          <thead className="bg-black text-xs !text-[#BD7F10]">
            <tr>
              {tableHeaders.map((header) => (
                <th
                  key={header}
                  className="border-b border-[#2a3038] px-4 py-3 text-left font-medium !text-[#BD7F10]"
                >
          ...
          <tbody>
            {pagedRows.map((row) => (
              <tr
                key={row.symbol}
                className="rounded-xl bg-[#1a1f2a] text-sm text-[#e7ebf4] ring-1 ring-[#252a36]/70 transition hover:bg-[#202633]"
              >
                ...
                <td className="px-4 py-3.5 align-middle">
                  ...
                  {row.totalUsdt ? (
                    <p className="mt-1 text-xs text-[#8892a8]">
                      {row.totalUsdt}
                    </p>
                  ) : (
      ...
      <div className="mt-4 flex items-center justify-center gap-2">
        <button
          ...
          className="h-6 min-w-6 rounded bg-black px-2 text-xs text-[#d2a13f] transition hover:bg-[#2b3447]"
        >
          {"<"}
        </button>
        {Array.from({ length: totalPages }).map((_, i) => {
          ...
          return (
            <button
              key={n}
              onClick={() => setPage(n)}
              className={`h-6 min-w-6 rounded px-2 text-xs transition ${active ? "bg-[#B87A13] text-[#131821]" : "bg-black text-[#d2a13f] hover:bg-[#2b3447]"}`}
            >
              {n}
            </button>
          );
